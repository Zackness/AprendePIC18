...
Un byte = 8 bits. En MPASM escribes valores con `B'10101010'` (binario), `D'170'` (decimal) o `0xAA` (hexadecimal). En apuntes e informes veras el mismo hex con **sufijo H**: `AAH`, `00H`, `FFH`. Para encender un bit concreto: `BSF`; para apagarlo: `BCF`. Para modificar varios bits a la vez sin tocar el resto: **mascaras** con AND, OR o XOR.
</AIExplain>
...
| Decimal | Binario | Hex (`0x`) | Hex (sufijo H) | Uso tipico |
| --- | --- | --- | --- | --- |
...
<AIExplain title="En MPASM y en el informe">
`MOVLW 0xC3`, `MOVLW 0C3h` y `MOVLW B'11000011'` cargan **el mismo valor** en `WREG`. En texto escrito puedes anotar **C3H** en lugar de `0xC3`. El informe puede pedir hex con sufijo H — no mezcles notaciones sin indicarlo.
</AIExplain>
