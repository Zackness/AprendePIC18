...
<Figure
	...
	caption="Full sequence: detect event, save context, run ISR, clear flag, return with RETFIE."
...
2. If the source is enabled and **GIE = 1**, the CPU jumps to the **vector** (`0x0008` high or `0x0018` low).
...
The ISR may use `WREG`, `STATUS`, and other registers the main program needed. **Course minimum:** save **WREG** and **STATUS** on entry and restore **before** `RETFIE`. Details in the [practical guide — context](/en/tutoriales/guias/interrupciones/3-contexto/).
...
In the lab, many practices use **high priority only** (`ORG 0x0008`) with all sources in `isr_high`.
...
For matrix keyboard ([RB interrupt guide](/en/tutoriales/guias/interrupciones/4-rb-interrupt/)):
