...
One byte = 8 bits. In MPASM you write `B'10101010'` (binary), `D'170'` (decimal), or `0xAA` (hex). In notes and reports you will see the same hex with **suffix H**: `AAH`, `00H`, `FFH`. To turn on a specific bit: `BSF`; to clear it: `BCF`. To change several bits at once without touching the rest: **masks** with AND, OR, or XOR.
</AIExplain>
...
| Decimal | Binary | Hex (`0x`) | Hex (H suffix) | Typical use |
| --- | --- | --- | --- | --- |
...
<AIExplain title="In MPASM and in reports">
`MOVLW 0xC3`, `MOVLW 0C3h`, and `MOVLW B'11000011'` load **the same value** into `WREG`. In written text you can write **C3H** instead of `0xC3`. Reports may ask for hex with H suffix — do not mix notations without stating it.
</AIExplain>
