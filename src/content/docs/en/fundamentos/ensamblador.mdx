...
Later in [MPLAB and Proteus](/en/fundamentos/mplab-proteus/) you will see the full toolchain. [Config bits and the configuration word](/en/fundamentos/config-bits/#what-is-the-configuration-word) go in the same `.ASM` but are stored in a special configuration area, not as normal executable code.
